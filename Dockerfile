...
RUN curl -O ftp://ftp.st.ryukoku.ac.jp/pub/network/ftp/lftp/lftp-4.7.4.tar.gz && tar xzvf lftp-4.7.4.tar.gz && cd lftp-4.7.4 && ./configure --with-gnutls --without-openssl --prefix=${HOME}/local && make && make install && mkdir ~/.lftp/ && echo "set ftp:ssl-force true;set ftp:ssl-protect-data true;set ftp:ssl-allow true;set ssl:verify-certificate no;" >> ~/.lftp/rc && ln -s ~/local/bin/lftp /usr/bin/lftp
